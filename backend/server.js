...
function makeJwksClient(jwksUri) {
  if (!jwksCache.has(jwksUri)) {
    jwksCache.set(jwksUri, jwksClient({
      jwksUri,
      cache: true,
      cacheMaxEntries: 10,
...
async function pickIssuerBase(req, tokenDec) {
  const headerIssuer = req.headers['x-ni-issuer'] ? String(req.headers['x-ni-issuer']) : null;
...
async function ensureAuth(req, res, next) {
  try {
    ...
    if (!issuer) {
      const hdr = req.headers['x-ni-issuer'] ? String(req.headers['x-ni-issuer']) : null;
      if (hdr) {
        const u = new URL(hdr);
        const bases = [
    ...
    const tryIssuers = [];
    if (req.headers['x-ni-issuer']) tryIssuers.push(String(req.headers['x-ni-issuer']));
    ...
    for (const cand of tryIssuers) {
      ...
      if (userInfo) {
        req.user = { sub: userInfo.id || userInfo.sub || userInfo.email, email: userInfo.email, iss };
        return next();
      }
    }

    return res.status(401).json({ ok: false, error: 'invalid token', hint: 'JWKS & /user validation failed' });
  } catch (e) {
...
app.get('/diag', async (_req, res) => {
  try {
    let driveOk = false, quota = null, user = nu
