// server.js — Shree Drive (STRICT PRIVATE)
// Requires env: ALLOWED_ORIGIN, IDENTITY_ISSUER, JWKS_URI, CLIENT_ID, CLIENT_SECRET, REDIRECT_URI, REFRESH_TOKEN, DRIVE_FOLDER_ID, MAKE_PUBLIC=false
...
async function ensurePublic(id) {
  ...
  try { await drive.permissions.create({ fileId: id, requestBody: { role: 'reader', type: 'anyone' } }); } catch {}
}
...
app.get('/diag', async (req, res) => {
  try {
    ...
    res.json({ ok:true, user:about.data.user, folder:DRIVE_FOLDER_ID||null, makePublic:MAKE_PUBLIC, issuer:IDENTITY_ISSUER, jwks:JWKS_URI });
...
app.post('/upload', requireAuth, upload.single('file'), async (req, res) => {
  try {
    ...
    await ensurePublic(r.data.id); // does nothing if MAKE_PUBLIC=false
