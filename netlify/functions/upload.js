...
function parseForm(event) {
  return new Promise((resolve, reject) => {
    ...
    // Netlify sends the raw body as a Buffer string. Convert to a stream.
    const bodyBuffer = Buffer.from(event.body, event.isBase64Encoded ? 'base64' : 'utf8');

    form.parse(
      // formidable can parse buffers if we pass an object with `headers` & `on`.
      { headers: event.headers,
        on: (name, cb) => { /* no-op to satisfy interface */ }
      },
      (err, fields, files) => {
